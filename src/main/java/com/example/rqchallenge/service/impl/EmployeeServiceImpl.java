...
/*
Implementation for employee related operations
 */
@Service
public class EmployeeServiceImpl implements EmployeeService {
	...
	public ResponseEntity<Employee> getEmployeeById(String id) {
		...
		try (Response response = client.newCall(request).execute()) {
			...
			logger.error("Error while retrieving employee details for id:{}", id, e);
		}
		return new ResponseEntity<Employee>(employee, HttpStatus.OK);
	}
	...
	public ResponseEntity<List<Employee>> getEmployeesByNameSearch(String searchString) {
		...
		try (Response response = client.newCall(request).execute()) {
			...
			employees = employees.stream().filter(x -> x.getEmployeeName().contains(searchString))
	...
	public ResponseEntity<Integer> getHighestSalaryOfEmployees() {
		...
		try (Response response = client.newCall(request).execute()) {
			...
			highestSalary = employees.stream().map(x -> x.getEmployeeSalary())
	...
	public ResponseEntity<List<String>> getTopTenHighestEarningEmployeeNames() {
		...
		try (Response response = client.newCall(request).execute()) {
			...
			employeeResponse = employees.stream().map(x -> x.getEmployeeName()).limit(10).collect(Collectors.toList());
		} catch (Exception e) {
